...
class DataBaseUser {
  ...
  const QUERY_READ_USER = "SELECT * FROM Cliente WHERE Mail = '%s' AND Password = MD5('%s');";
	...
	const QUERY_UPDATE_USER = "UPDATE Cliente SET Nombre = '%s', PrimerApellido = '%s', Celular = '%s', Mail = '%s' WHERE Mail = '%s'";
  ...
  public function readSession($token)
  {
	...
	public function updateUser($newName, $newLastName, $newCel, $newMail, $oldMail)
	{
		$query = sprintf(DataBaseUser::QUERY_UPDATE_USER, $newName, $newLastName, $newCel, $newMail, $oldMail);
		if(!$this->mysqli->query($query))
			throw new errorWithDatabaseException("ERROR");
	}
}
