...
class Service {
	...
	public function changeServiceStatus($serviceId, $statusId, $cancelCode) {
		...
		if ($statusId == 4) {
			...
			$this->dataBase->generarYGuardarCodigoDePagoPorEfectivo($serviceId);
		}
		$line = $this->getInfo ( $serviceId );
	...
	function makePayment($serviceId, $line, $price) {
		if ($line ['idTransaccion'] == null && $line['pago'] == "t") {
	...
	public function requestService($direccion, $latitud, $longitud, $idServicio, $idCliente, $idCoche, $idCocheFavorito, $metodoDePago) {
		if ($this->dataBase->usuarioEstaBloqueado($idCliente)) {
			throw new usuarioBloqueado();
		}
		date_default_timezone_set ( 'America/Mexico_City' );
		$fecha = date ( "Y-m-d H:i:s" );
		$idService = $this->dataBase->insertService ( $fecha, $direccion, $latitud, $longitud, $idServicio, $idCliente, $idCoche , $idCocheFavorito, $metodoDePago);
		return $idService;
	}
