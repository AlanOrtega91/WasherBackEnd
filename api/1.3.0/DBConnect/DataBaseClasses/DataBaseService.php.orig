...
class DataBaseService extends DataBase {
  ...
  const QUERY_READ_ALL_SERVICES = "SELECT * FROM Servicio;";
	...
	idCliente, idVehiculo, idVehiculoFavorito, metodoDePago, precioAPagar)
  ...
  const QUERY_READ_SERVICE =
	...
	public function insertService($fecha,$direccion, $latitud,$longitud,$idServicio,$idCliente,$idCoche, $idCocheFavorito, $metodoDePago, $precio, $precioAPagar)
	{
		$query = sprintf(DataBaseService::QUERY_INSERT_SERVICE,$fecha,$direccion,$latitud,$longitud,$precio,
				$idServicio,$idCliente,$idCoche, $idCocheFavorito, $metodoDePago, $precioAPagar);
		$this->ejecutarQuery($query);
