...
class Service {
	...
	public function requestService($direccion, $latitud, $longitud, $idServicio, $idCliente, $idCoche, $idCocheFavorito, $metodoDePago) {
		...
		date_default_timezone_set ( 'America/Mexico_City' );
		$fecha = date ( "Y-m-d H:i:s" );
		$precio = $this->dataBase->calculatePrice($idCoche, $idServicio);
		$precioAPagar = $this->calculaPrecioAPagar($precio,$idCliente);
		$idService = $this->dataBase->insertService ( $fecha, $direccion, $latitud, $longitud, $idServicio, $idCliente, $idCoche , $idCocheFavorito, $metodoDePago, $precio, $precioAPagar);
		return $idService;
	}
