...
class DataBaseUser extends DataBase{
	...
	WHERE Token = '%s';";
	const QUERY_READ_USER_BY_ID = "SELECT * FROM Cliente WHERE idCliente = %s";
	...
	const QUERY_INSERT_USER = "INSERT INTO Cliente (Nombre, PrimerApellido, Email, Password, Telefono, codigo, ConektaId) VALUES ('%s', '%s', '%s', SHA2(MD5(('%s')),512), '%s', CONCAT(FLOOR(RAND()*10), LEFT(MD5(RAND()),4)), '%s')";
	...
	public function insertNewUser($name, $lastName, $email, $password, $phone, $conektaId) {
		$query = sprintf ( DataBaseUser::QUERY_INSERT_USER, $name, $lastName, $email, $password, $phone , $conektaId);
		$this->ejecutarQuery($query);
	}
	...
	public function readUserInfoById($id) {
	    $query = sprintf ( DataBaseUser::QUERY_READ_USER_BY_ID, $id );
	    $result = $this->ejecutarQuery($query);
